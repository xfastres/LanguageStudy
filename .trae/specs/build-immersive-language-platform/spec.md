...
#### Scenario: MVP 阶段
- **WHEN** MVP 上线
- **THEN** 包含：用户注册登录、英语内容库（Level 0-2）、基础内容消费（视频+音频）、输入增强（字幕+悬停释义+语速调节）、Language Acquisition Engine 基础版（i+1 匹配+行为信号推断+State Machine State 0-2）、Content Pipeline 基础版（ASR+分级+向量化）、Language Companion Agent 基础版、Comprehension Analyzer Agent 基础版、Silent Period Guardian Agent 基础版、Runtime Constitution Rule 1-8 基础执行、听力时间统计、静默期保护、North Star Metrics 追踪基础版

#### Scenario: V2 阶段
- **WHEN** V2 上线
- **THEN** 新增：日语/韩语支持、AI 对话输出系统、单词习得系统、Content Adaptation Agent、社区沉浸、成就系统、高级可视化、AI-native 内容生成、State Machine State 3-4 支持

#### Scenario: V3 阶段
- **WHEN** V3 上线
- **THEN** 新增：输出系统完整版、AI 角色世界、内容创作者平台、高级 AI 推荐引擎、Philosophical Guardrails 自动审查工具、Runtime Constitution 完整执行与监控、State Machine State 5 支持、商业化完整方案
