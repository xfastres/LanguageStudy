# LinguaFlow OS — AI-native Cognitive Immersion System Spec
...
LinguaFlow OS 本质上不是 EdTech 产品，不是语言学习平台，而是一套 **AI-native Cognitive Immersion System（认知沉浸系统）**。它模拟母语习得过程，通过持续沉浸让用户自然获得语言能力。用户不是在"学语言"，而是在认知沉浸中自然习得语言。
...
MVP 不是功能列表，而是一个**完整的用户价值闭环**。最小闭环必须验证：用户是否能在 LinguaFlow 中通过沉浸自然习得语言。

### 最小闭环定义

用户从注册到体验到"自然理解了之前不懂的内容"，这个完整路径就是 MVP。

### MVP 最小闭环路径

1. **注册** → 选择学习语言 → 兴趣标签 → 自评理解等级
2. **初始画像** → 系统生成初始语言画像 → 判定初始 State
3. **首次沉浸** → 推荐第一个 i+1 内容 → 用户消费内容
4. **行为信号采集** → 系统采集暂停/回放/完成率等行为信号
5. **画像更新** → 基于行为信号更新语言画像与情绪状态
6. **推荐调整** → 基于更新后的画像调整下一内容推荐
7. **自然理解验证** → 用户在后续内容中自然理解了之前不懂的表达
...
- 视频播放器（播放/暂停/字幕/语速调节）
- Language Acquisition Engine 基础版（i+1 匹配 + 行为信号推断）
- State Machine State 0-2
- Acquisition Memory Graph 基础版（词汇暴露追踪）
- Emotional State Engine 基础版（焦虑/专注度检测）
- Immersion Runtime 基础版（沉浸检测 + 静默队列）
- Runtime Constitution Rule 1-8 基础执行
- North Star Metrics 基础追踪（有效可理解输入时长）

### MVP 不包含的模块

- OAuth 登录、AI 对话、社区、成就系统、Content Pipeline 完整版、AI-native 内容生成、State 3-5、Agent 完整版
...
### Moat 4：Emotional State Engine 情绪模型

情绪状态引擎的训练数据来自真实用户的语言习得情绪反应，这个数据集在市场上不存在，只能通过 LinguaFlow 自身积累。

### Moat 5：Guardrails 防退化系统

Guardrails + Runtime Constitution + Non-Goals 守卫形成的产品防退化系统，是组织级壁垒。大多数竞品会因为 DAU/营收压力逐渐退化回传统教育，而 LinguaFlow 的护栏系统从架构层面阻止了这种退化。
...
系统 SHALL 按阶段交付。MVP 以最小闭环为核心，验证"用户通过沉浸自然习得语言"的核心假设。

#### Scenario: MVP 阶段（最小闭环）
- **WHEN** MVP 上线
- **THEN** 包含最小闭环路径：注册→画像→首次沉浸→行为信号采集→画像更新→推荐调整→自然理解验证
- **AND** 包含模块：邮箱注册、英语内容库（Level 0-2，50+ 条）、视频播放器（播放/暂停/字幕/语速）、Language Acquisition Engine 基础版、State Machine State 0-2、Acquisition Memory Graph 基础版、Emotional State Engine 基础版、Immersion Runtime 基础版、Runtime Constitution Rule 1-8 基础执行、North Star Metrics 基础追踪

#### Scenario: V2 阶段
- **WHEN** V2 上线
- **THEN** 新增：OAuth 登录、日语/韩语支持、AI 对话输出系统、单词习得系统、Content Adaptation Agent、社区沉浸、成就系统、高级可视化、AI-native 内容生成、State Machine State 3-4、Acquisition Memory Graph 完整版、Emotional State Engine 完整版

#### Scenario: V3 阶段
- **WHEN** V3 上线
- **THEN** 新增：输出系统完整版、AI 角色世界、内容创作者平台、高级 AI 推荐引擎、Philosophical Guardrails 自动审查工具、Runtime Constitution 完整执行与监控、State Machine State 5、Immersion Runtime 完整版、推荐系统防污染架构完整版、商业化完整方案
