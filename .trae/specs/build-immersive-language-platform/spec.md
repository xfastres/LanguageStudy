...
MVP 不是功能列表，而是一个**完整的用户价值闭环**。最小闭环必须验证一个生死问题：**用户会不会真的连续沉浸 1 小时以上？**
...
1. **内容流**：TikTok-like 无限滚动，Level 0-2 英语内容，至少 50 条
2. **i+1 Engine**：基于 pause / replay / completion / subtitle usage 动态调节
3. **内容分级**：Level 0-2 分级逻辑
4. **简单画像**：兴趣标签 + 自评理解力
5. **Effective Input Hours**：唯一指标——有效可理解输入时长

### 真 MVP 必须删除

- 社区、成就、AI Companion、多语言
- Content Pipeline 完整版、Emotional Engine 完整版
- Agent 群、Runtime Constitution 完整版
- 世界系统、UGC、AI-native 内容
- Ambient System、Identity Engine、Narrative Engine

### 验证目标

用户会不会真的连续沉浸 1 小时以上。这是生死问题。不是 Agent，不是 AI，不是世界观。

### MVP 之后的增量路径

- MVP+1：加入 State Machine State 0-2 + Acquisition Memory Graph 基础版
- MVP+2：加入 Emotional State Engine 基础版 + Immersion Runtime 基础版
- MVP+3：加入第一个 World（如 Tokyo Apartment）+ Companion 基础版
...
**核心信号 4**：用户是否开始在脑中出现目标语言内心独白（终极信号）
